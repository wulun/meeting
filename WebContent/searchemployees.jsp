...
<script type="text/javascript">
	...
	function goToOnePage(employeename, username, status) {
		...
		}else if(!patrn.test(pageNum)){
			alert("页码不存在!");
		}
		
		else{
			window.location.href = "SearchEmployeesServlet?employeename=" + employeename + "&username=" + username + "&status=" + status
					+ "&pageNum=" + pageNum;
		}
...
<body>
	<div class="page-content">
		...
		<form method="post" action="SearchEmployeesServlet?pageNum=1">
			<fieldset>
				<legend>搜索员工</legend>
				<table class="formtable">
					<tr>
						<td>姓名：</td>
						<td><input type="text" id="employeename" name="employeename" value="${param.employeename}" maxlength="20" /></td>
						<td>账号名：</td>
						<td><input type="text" id="username" name="username" value="${param.username}" maxlength="20" /></td>
						<td>状态：</td>
					...
					<tr>
						<td colspan="6" class="command"><input type="submit" class="clickbutton" value="查询" /> 
						<input type="button" class="clickbutton" value="重置" onclick="reset1()"/></td>
					</tr>
				</table>
			</fieldset>
		</form>
			...
			<div>
				...
				<div class="pager-header">
					<div class="header-info">
						共<span class="info-number">${requestScope.countOfEmployees}</span>
						<input type="hidden" value="${requestScope.countOfPages}" id = "PN">条结果，
						分成<span class="info-number">${requestScope.countOfPages}</span>页显示，
						当前第<span class="info-number">${requestScope.pageNum}</span>页.
						<font style="color:red "><span>${requestScope.msg}</span></font>
					</div>
